...
\begin{quotation}
	El \textit{machine learning} fue un objeto de lujo, pero para nosotros es un artículo de primera necesidad: no podemos vivir sin \textit{machine learning}.
...
\caption{\subref{fig:grafo_ejemplo} Ejemplo de modelo gráfico dirigido. \subref{fig:naive_bayes} Relaciones de dependencia condicional en el clasificador naive Bayes como un modelo gráfico dirigido.}
\end{figure}
\begin{figure}[ht]
\centering
\subfloat[]{\label{fig:plate_naive_bayes} \input{./figuras/plate_naive_bayes_graph}}
\caption{\subref{fig:plate_naive_bayes} Clasificador naive Bayes con notación de placas.}
...
posea una numeración inferior a sus nodos hijos. \todo{Agregar lemma de ordenamiento topologico
para DAG's en apendice.} Esta característica permite enriquecer la formulación de la propiedad
...
clasificación conocido como \index{clasificador naive Bayes}\textbf{clasificador naive Bayes}. La estructura de las relaciones de independencia inducidas por (\ref{eq:cond_nb}) se pueden
...
Sea $\mathcal{M}$ un modelo grafico dirigido, en el cual todas las variables son reales y sus distribuciones de probabilidad
condicional son lineal-gaussianas:
\begin{equation}
...
Lo cual se conoce como \index{red bayesiana gaussiana} \textbf{red bayesiana gaussiana}. Para este tipo de modelos, es posible inferir
...
Es posible entonces, encontrar de manera recursiva las componentes de $\mathbb{E}[\bm x] =(\mathbb{E}[x_1], \ldots, \mathbb{E}[x_V])^T$ utilizando la estructura gráfica de $\mathcal{M}$. Para ello, se comienza calculando $\mathbb{E}[x_1]$ para luego continuar
