...
\subref{fig:naive_bayes} Relaciones de dependencia condicional en el clasificador
 naive Bayes como un modelo gráfico dirigido.}
\end{figure}
\begin{figure}[ht]
\centering
...
En general, es posible reconstruir la probabilidad conjunta subyacente a un modelo
 gráfico probabilístico conociendo el grafo y haciendo el proceso inverso al descrito anteriormente.
\end{example}
...
Es posible entonces, encontrar de manera recursiva las componentes de $\mathbb{E}[\bm x]
=(\mathbb{E}[x_1], \ldots, \mathbb{E}[x_V])^T$ utilizando la estructura gráfica
de $\mathcal{M}$. Para ello, se comienza calculando $\mathbb{E}[x_1]$ para luego continuar
